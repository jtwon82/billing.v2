...
@Configuration
@Service
public class ConsumerProcessBranch {
...
//			PollingData pollData = new ObjectMapper().readValue(message, PollingData.class);
			try {
				className = (String) jSONObject.get("className");
//				className= pollData.getClassName();
			} catch (Exception e) {
			...
			if (G.ClickViewData.equals(topic) || G.ClickViewPointData.equals(topic)) {
				...
				if (record != null) {
					...
					else
					{
						...
						} else if ("BranchUM".equals(this.profileId)) {

							if ("UM".equals(record.getAdGubun())) {
								BaseCVData knoSiteCodeData = record.clone();
								BaseCVData knoScriptNoData = record.clone();
								this.sumObjectManager.appendKnoUMSiteCodeData(knoSiteCodeData);
								this.sumObjectManager.appendKnoUMScriptNoData(knoScriptNoData);
							}
//							BaseCVData knoKpiData = this.selectDao.selectNewKpiInfo(record);
	//
//							try{
//								this.sumObjectManager.appendKnoKpiData(knoKpiData);
//							}catch(Exception ee) {
//								logger.info("knoKpiData sc:{}", knoKpiData.getSiteCode());
//								logger.error("ee ", ee);
//							}
			...
			} else if (G.ExternalData.equals(topic)) {} else {
