package com.mobon.billing.env.service.dao;
...
@Repository
public class CampMediaRetrnAvalDataDao {

	private static final Logger	logger	= LoggerFactory.getLogger(CampMediaRetrnAvalDataDao.class);
			...
			public Object doInTransaction(TransactionStatus status) {
				try {
					...
					for (Entry<String, ArrayList<BaseCVData>> item : flushMap.entrySet()) {
						...
						logger.debug("CAMPMEDIARETRNAVAL dataKey - {}", dataKey);
						logger.debug("CAMPMEDIARETRNAVAL dataSize - {}", dataSize);
					...
					if(totalBillingSize>0) {
						...
						logger.info("TR Time (TBRT) Billing CampMediaRetrnAval : " + resutTime + "(ms) totalsize - "+ totalBillingSize);
					}
