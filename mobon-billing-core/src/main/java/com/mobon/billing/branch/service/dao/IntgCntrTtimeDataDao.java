...
@Repository
public class IntgCntrTtimeDataDao {

	private static final Logger	logger	= LoggerFactory.getLogger(IntgCntrTtimeDataDao.class);

	public static final String	NAMESPACE	= "intgCntrTtimeDataMapper";
			...
			public Object doInTransaction(TransactionStatus status) {
				try {
					...
					for (Entry<String, ArrayList<BaseCVData>> item : flushMap.entrySet()) {
						...
						logger.debug("IntgCntrTtime dataKey - {}", dataKey);
						logger.debug("IntgCntrTtime dataSize - {}", dataSize);
					...
					if(totalBillingSize>0) {
						...
						logger.info("TR Time (TBRT) Billing IntgCntrTtime : " + resutTime + "(ms) totalsize - "+ totalBillingSize);
					}
