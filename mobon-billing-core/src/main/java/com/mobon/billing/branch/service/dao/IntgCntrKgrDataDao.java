...
@Repository
public class IntgCntrKgrDataDao {

	private static final Logger	logger	= LoggerFactory.getLogger(IntgCntrKgrDataDao.class);

	public static final String	NAMESPACE	= "intgCntrKgrDataMapper";
			...
			public Object doInTransaction(TransactionStatus status) {
				try {
					...
					for (Entry<String, ArrayList<BaseCVData>> item : flushMap.entrySet()) {
						...
						logger.debug("IntgCntrKgr dataKey - {}", dataKey);
						logger.debug("IntgCntrKgr dataSize - {}", dataSize);
					...
					if(totalBillingSize>0) {
						...
						logger.info("TR Time (TBRT) Billing IntgCntrKgr : " + resutTime + "(ms) totalsize - "+ totalBillingSize);
					}
