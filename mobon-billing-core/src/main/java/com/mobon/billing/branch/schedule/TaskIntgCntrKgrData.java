...
import com.mobon.billing.branch.service.IntgCntrKgrDataToMariaDB;
...
@Component
public class TaskIntgCntrKgrData {

	private static final Logger		logger		= LoggerFactory.getLogger(TaskIntgCntrKgrData.class);

	@Autowired
	private SelectDao				selectDao;
	
	@Autowired
	private IntgCntrKgrDataToMariaDB	intgCntrKgrDataToMariaDB;
	...
	public static void setThreadCnt(int threadCnt) {
		TaskIntgCntrKgrData.threadCnt = threadCnt;
	}
	public static synchronized void increaseThreadCnt() {
		TaskIntgCntrKgrData.threadCnt++;
	}
	...
	public static int getThreadCnt() {	
		return TaskIntgCntrKgrData.threadCnt;
	}
	
	
	
	public void mongoToFileIntgCntrKgr() {
		Map<String, BaseCVData> summeryBaseCVData = (Map<String, BaseCVData>) sumObjectManager.removeIntgCntrKgrMap();
		...
		if(summeryBaseCVData!=null && summeryBaseCVData.entrySet()!=null) {
			...
			if( listWriteData.size()>0 ) {
				...
				try {
					ConsumerFileUtils.writeLine( logPath +"retry/", writeFileName, G.IntgCntrKgrData, listWriteData);
				} catch (IOException e) {
				}
			}
		}
	...
	public void mongoToMariaIntgCntrKgrV3() {
		logger.info(">> START mongoToMariaIntgCntrKgrV3 THREAD COUNT - {}", threadCnt);
		...
		Map<String, BaseCVData> summeryBaseCVData = (Map<String, BaseCVData>) sumObjectManager.removeIntgCntrKgrMap();
		ArrayList<BaseCVData> listIntgCntrKgrData = new ArrayList();
		if(summeryBaseCVData!=null && summeryBaseCVData.entrySet()!=null) {
			...
			for (String[] group : group_key) {
				...
				List<BaseCVData> filtering = (List<BaseCVData>) listIntgCntrKgrData.stream()
				...
				if( filtering.size()>0 ) {
					...
					try {
						if ( workingKey.contains(_id) ) {
							logger.info("workingKey.contains - {}, listIntgCntrKgrData.size - {}", _id, listIntgCntrKgrData.size());
							
							for( BaseCVData row : filtering ) {
								sumObjectManager.appendIntgCntrKgrData(row);
							}
						} else {
							workingKey.add( _id, 3 );
							
							if( 10000>filtering.size() ) {
								workQueue.execute(new TaskData(G.IntgCntrKgrData, _id, filtering));
								...
								workQueue.execute(new TaskData(G.IntgCntrKgrData, _id, intoFiltering));
							}
						}
					}catch(Exception e) {
						for( BaseCVData row : filtering ) {
	...
	public void mongoToMariaV3(TaskData taskData) {
		...
		try {
			if ( taskData.increaseRetryCnt() < retryConfig.maxRetryCount ) {
				result = intgCntrKgrDataToMariaDB.intoMariaIntgCntrKgrDataV3(taskData.getId(), (List<BaseCVData>)taskData.getFiltering(), false);
			} else {
				result = intgCntrKgrDataToMariaDB.intoMariaIntgCntrKgrDataV3(taskData.getId(), (List<BaseCVData>)taskData.getFiltering(), true);
			}
		} catch (Exception e) {
