...
import com.mobon.billing.core.service.AdverClientEnvirmentDataToMariaDB;
import com.mobon.billing.core.service.ClientEnvirmentDataToMariaDB;
...
@Component
public class TaskAdverClientEnvirmentData {

	private static final Logger		logger		= LoggerFactory.getLogger(TaskAdverClientEnvirmentData.class);

	@Autowired
	private AdverClientEnvirmentDataToMariaDB	adverClientEnvirmentDataToMariaDB;
	...
	public static void setThreadCnt(int threadCnt) {
		TaskAdverClientEnvirmentData.threadCnt = threadCnt;
	}
	public static synchronized void increaseThreadCnt() {
		TaskAdverClientEnvirmentData.threadCnt++;
	}
	...
	public static int getThreadCnt() {	
		return TaskAdverClientEnvirmentData.threadCnt;
	}
	
	
	
	public void mongoToFileAdverClientEnvirment() {
		Map<String, BaseCVData> summeryBaseCVData = (Map<String, BaseCVData>) sumObjectManager.removeAdverClientEnvirmentObjectMap(new BaseCVData());
		...
		if(summeryBaseCVData!=null && summeryBaseCVData.entrySet()!=null) {
			...
			if( listWriteData.size()>0 ) {
				...
				try {
					ConsumerFileUtils.writeLine( logPath +"retry/", writeFileName, G.adver_client_environment, listWriteData);
				} catch (IOException e) {
				}
			}
		}
	...
	public void mongoToMariaAdverClientEnvirmentV3() {
		logger.info(">> START mongoToMariaAdverClientEnvirmentV3 THREAD COUNT - {}", threadCnt);
...
//					} else {
						...
						if( 10000>listBaseCVData.size() ) {
							workQueue.execute(new TaskData(G.adver_client_environment, _id, listBaseCVData));
							...
							for( BaseCVData row : listBaseCVData ) {
								if( 10000>cnt++ ) {
									...
									sumObjectManager.appendAdverClientEnvironmentData(row);
								}
							}
							workQueue.execute(new TaskData(G.adver_client_environment, _id, intoFiltering));
						}
					}
//				}
//			}
	...
	public void mongoToMariaV3(TaskData taskData) {
		...
		try {
			if ( taskData.increaseRetryCnt() < retryConfig.maxRetryCount ) {
				result = adverClientEnvirmentDataToMariaDB.intoMariaAdverClientEnvirmentDataV3(taskData.getId(), (List<BaseCVData>)taskData.getFiltering(), false);
			} else {
				result = adverClientEnvirmentDataToMariaDB.intoMariaAdverClientEnvirmentDataV3(taskData.getId(), (List<BaseCVData>)taskData.getFiltering(), true);
			}
		} catch (Exception e) {
