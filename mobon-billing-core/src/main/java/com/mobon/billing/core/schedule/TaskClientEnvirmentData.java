...
@Component
public class TaskClientEnvirmentData {

	private static final Logger		logger		= LoggerFactory.getLogger(TaskClientEnvirmentData.class);

	@Autowired
	private ClientEnvirmentDataToMariaDB	clientEnvirmentDataToMariaDB;
	...
	public static void setThreadCnt(int threadCnt) {
		TaskClientEnvirmentData.threadCnt = threadCnt;
	}
	public static synchronized void increaseThreadCnt() {
		TaskClientEnvirmentData.threadCnt++;
	}
	...
	public static int getThreadCnt() {	
		return TaskClientEnvirmentData.threadCnt;
	}
	
	
	
	public void mongoToFileClientEnvirment() {
		Map<String, BaseCVData> summeryBaseCVData = (Map<String, BaseCVData>) sumObjectManager.removeClientEnvirmentObjectMap(new BaseCVData());
		...
		if(summeryBaseCVData!=null && summeryBaseCVData.entrySet()!=null) {
			...
			if( listWriteData.size()>0 ) {
				...
				try {
					ConsumerFileUtils.writeLine( logPath +"retry/", writeFileName, G.client_environment, listWriteData);
				} catch (IOException e) {
				}
			}
		}
	...
	public void mongoToMariaClientEnvirmentV3() {
		logger.info(">> START mongoToMariaClientEnvirmentV3 THREAD COUNT - {}", threadCnt);
...
//					} else {
						...
						if( 10000>listBaseCVData.size() ) {
							workQueue.execute(new TaskData(G.client_environment, _id, listBaseCVData));
							...
							for( BaseCVData row : listBaseCVData ) {
								if( 10000>cnt++ ) {
									...
									sumObjectManager.appendClientEnvironmentData(row);
								}
							}
							workQueue.execute(new TaskData(G.client_environment, _id, intoFiltering));
						}
					}
//				}
//			}
	...
	public void mongoToMariaV3(TaskData taskData) {
		...
		try {
			if ( taskData.increaseRetryCnt() < retryConfig.maxRetryCount ) {
				result = clientEnvirmentDataToMariaDB.intoMariaClientEnvirmentDataV3(taskData.getId(), (List<BaseCVData>)taskData.getFiltering(), false);
			} else {
				result = clientEnvirmentDataToMariaDB.intoMariaClientEnvirmentDataV3(taskData.getId(), (List<BaseCVData>)taskData.getFiltering(), true);
			}
		} catch (Exception e) {
