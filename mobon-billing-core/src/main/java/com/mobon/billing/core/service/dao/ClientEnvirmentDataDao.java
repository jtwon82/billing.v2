...
@Repository
public class ClientEnvirmentDataDao {

	private static final Logger	logger	= LoggerFactory.getLogger(ClientEnvirmentDataDao.class);
			...
			public Object doInTransaction(TransactionStatus status) {
				try {
					...
					for (Entry<String, ArrayList<BaseCVData>> item : flushMap.entrySet()) {
						...
						logger.debug("CLIENTENVIRMENT dataKey - {}", dataKey);
						logger.debug("CLIENTENVIRMENT dataSize - {}", dataSize);
					...
					if(totalBillingSize>0) {
						...
						logger.info("TR Time (TBRT) Billing ClientEnvirment : " + resutTime + "(ms) totalsize - "+ totalBillingSize);
					}
