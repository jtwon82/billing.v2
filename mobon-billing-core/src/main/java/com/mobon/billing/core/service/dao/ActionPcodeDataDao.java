...
@Repository
public class ActionPcodeDataDao {
	
	private static final Logger	logger	= LoggerFactory.getLogger(ActionPcodeDataDao.class);
			...
			public Object doInTransaction(TransactionStatus status) {
				...
				try {
			        ...
			        if(totalsize>0) {
						...
						logger.info("TR Time (TBRT) Dream ActionPcodeData : " + resutTime + "(ms) totalsize - "+ totalsize);
			        }
