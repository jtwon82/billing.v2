...
import com.mobon.billing.core.service.ClickViewPointDataToMariaDB;
...
@Component
public class TaskClickViewPointData {

	private static final Logger		logger		= LoggerFactory.getLogger(TaskClickViewPointData.class);

	@Autowired
	private SelectDao				selectDao;
	
	@Autowired
	private ClickViewPointDataToMariaDB	clickviewPointDataToMariaDB;
	...
	public static void setThreadCnt(int threadCnt) {
		TaskClickViewPointData.threadCnt = threadCnt;
	}
	public static synchronized void increaseThreadCnt() {
		TaskClickViewPointData.threadCnt++;
	}
	...
	public static int getThreadCnt() {	
		return TaskClickViewPointData.threadCnt;
	}
	
	
	
	public void mongoToFileClickViewPoint() {
		Map<String, BaseCVData> summeryBaseCVData = (Map<String, BaseCVData>) sumObjectManager.removePointObjectMap(new BaseCVData());
		...
		if(summeryBaseCVData!=null && summeryBaseCVData.entrySet()!=null) {
			...
			if( listWriteData.size()>0 ) {
				...
				try {
					ConsumerFileUtils.writeLine( logPath +"retry/", writeFileName, G.click_view_point, listWriteData);
				} catch (IOException e) {
				}
			}
		}
	...
	public void mongoToMariaClickViewPointV3() {
		logger.info(">> START mongoToMariaClickViewPointV3 THREAD COUNT - {}", threadCnt);
		...
		Map<String, BaseCVData> summeryBaseCVPointData = (Map<String, BaseCVData>) sumObjectManager.removePointObjectMap(new BaseCVData() );
		ArrayList<BaseCVData> listBaseCVData = new ArrayList();
		if(summeryBaseCVPointData!=null && summeryBaseCVPointData.entrySet()!=null) {
			Iterator<Entry<String, BaseCVData>> it = summeryBaseCVPointData.entrySet().iterator();
			while (it.hasNext()) {
				try {
			...
			if(listBaseCVData.size()>0) {
				...
				if ( workingKey.contains(_id) ) {
					...
					for( BaseCVData row : listBaseCVData ) {
						sumObjectManager.appendPointData(row, false);
					}
				} else {
					...
					if( 10000>listBaseCVData.size() ) {
						workQueue.execute(new TaskData(G.click_view_point, _id, listBaseCVData));
						...
						workQueue.execute(new TaskData(G.click_view_point, _id, intoFiltering));
					}
				}
			}
...
//							workQueue.execute(new TaskData(G.click_view_point, _id, filtering));
//							
//						} else {
//							List<BaseCVData> intoFiltering = new ArrayList();
//							int cnt=0;
	...
	public void mongoToMariaV3(TaskData taskData) {
		...
		try {
			if ( taskData.increaseRetryCnt() < retryConfig.maxRetryCount ) {
				result = clickviewPointDataToMariaDB.intoMariaClickViewPointDataV3(taskData.getId(), (List<BaseCVData>)taskData.getFiltering(), false);
			} else {
				result = clickviewPointDataToMariaDB.intoMariaClickViewPointDataV3(taskData.getId(), (List<BaseCVData>)taskData.getFiltering(), true);
			}
		} catch (Exception e) {
