...
@Repository
public class ViewPcodeDataDao {
	
	private static final Logger	logger	= LoggerFactory.getLogger(ViewPcodeDataDao.class);
			...
			public Object doInTransaction(TransactionStatus status) {
				try {
					...
					for (Entry<String, ArrayList<BaseCVData>> item : flushMap.entrySet()) {
						...
						logger.debug("VIEW dataKey - {}", dataKey);
						logger.debug("VIEW dataSize - {}", dataSize);
