...
@Repository
public class AdverClientEnvirmentDataDao {

	private static final Logger	logger	= LoggerFactory.getLogger(AdverClientEnvirmentDataDao.class);
			...
			public Object doInTransaction(TransactionStatus status) {
				try {
					...
					for (Entry<String, ArrayList<BaseCVData>> item : flushMap.entrySet()) {
						...
						logger.debug("ADVERCLIENTENVIRMENT dataKey - {}", dataKey);
						logger.debug("ADVERCLIENTENVIRMENT dataSize - {}", dataSize);
					...
					if(totalBillingSize>0) {
						...
						logger.info("TR Time (TBRT) Billing AdverClientEnvirment : " + resutTime + "(ms) totalsize - "+ totalBillingSize);
					}
