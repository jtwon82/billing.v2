...
import com.mobon.billing.core.service.CrossAUIDDataToMariaDB;
...
@Component
public class TaskCrossAUIDData {

	private static final Logger		logger		= LoggerFactory.getLogger(TaskCrossAUIDData.class);

	@Autowired
	private SelectDao				selectDao;
	
	@Autowired
	private CrossAUIDDataToMariaDB crossAUIDDataToMariaDB;
	...
	public static void setThreadCnt(int threadCnt) {
		TaskCrossAUIDData.threadCnt = threadCnt;
	}
	public static synchronized void increaseThreadCnt() {
		TaskCrossAUIDData.threadCnt++;
	}
	...
	public static int getThreadCnt() {	
		return TaskCrossAUIDData.threadCnt;
	}

	public void mongoToMariaCrossAUIDV3() {
		logger.info(">> START mongoToMariaCrossAUIDV3 THREAD COUNT - {}", threadCnt);
		...
		Map<String, BaseCVData> summeryBaseCVData = (Map<String, BaseCVData>) sumObjectManager.removeCrossAUIDObjectMap(new BaseCVData() );
...
//					} else {
				...
				if( 10000>listBaseCVData.size() ) {
					workQueue.execute(new TaskData(G.click_view_crossAUID, _id, listBaseCVData));
					...
					for( BaseCVData row : listBaseCVData ) {
						if( 10000>cnt++ ) {
							...
							sumObjectManager.appendCrossAUIDData(row);
						}
					}
					workQueue.execute(new TaskData(G.click_view_crossAUID, _id, intoFiltering));
				}
			}
//				}
		}
	...
	public void mongoToMariaV3(TaskData taskData) {
		...
		try {
			if ( taskData.increaseRetryCnt() < retryConfig.maxRetryCount ) {
				result = crossAUIDDataToMariaDB.intoMariaCrossAUIDDataV3(taskData.getId(), (List<BaseCVData>)taskData.getFiltering(), false);
			} else {
				result = crossAUIDDataToMariaDB.intoMariaCrossAUIDDataV3(taskData.getId(), (List<BaseCVData>)taskData.getFiltering(), true);
			}
		} catch (Exception e) {
