...
import com.mobon.billing.core.service.ClientAgeGenderDataToMariaDB;
...
@Component
public class TaskClientAgeGenderData {

	private static final Logger		logger		= LoggerFactory.getLogger(TaskClientAgeGenderData.class);

	@Autowired
	private ClientAgeGenderDataToMariaDB	clientAgeGenderDataToMariaDB;
	...
	public static void setThreadCnt(int threadCnt) {
		TaskClientAgeGenderData.threadCnt = threadCnt;
	}
	public static synchronized void increaseThreadCnt() {
		TaskClientAgeGenderData.threadCnt++;
	}
	...
	public static int getThreadCnt() {	
		return TaskClientAgeGenderData.threadCnt;
	}
	
	
	
	public void mongoToFileClientAgeGender() {
		Map<String, BaseCVData> summeryBaseCVData = (Map<String, BaseCVData>) sumObjectManager.removeClientAgeGenderObjectMap(new BaseCVData());
		...
		if(summeryBaseCVData!=null && summeryBaseCVData.entrySet()!=null) {
			...
			if( listWriteData.size()>0 ) {
				...
				try {
					ConsumerFileUtils.writeLine( logPath +"retry/", writeFileName, G.client_age_gender, listWriteData);
				} catch (IOException e) {
				}
			}
		}
	...
	public void mongoToMariaClientAgeGenderV3() {
		logger.info(">> START mongoToMariaClientAgeGenderV3 THREAD COUNT - {}", threadCnt);
...
//					} else {
						...
						if( 10000>listBaseCVData.size() ) {
							workQueue.execute(new TaskData(G.client_age_gender, _id, listBaseCVData));
							...
							for( BaseCVData row : listBaseCVData ) {
								if( 10000>cnt++ ) {
									...
									sumObjectManager.appendClientAgeGenderData(row);
								}
							}
							workQueue.execute(new TaskData(G.client_age_gender, _id, intoFiltering));
						}
					}
//				}
//			}
	...
	public void mongoToMariaV3(TaskData taskData) {
		...
		try {
			if ( taskData.increaseRetryCnt() < retryConfig.maxRetryCount ) {
				result = clientAgeGenderDataToMariaDB.intoMariaClientAgeGenderDataV3(taskData.getId(), (List<BaseCVData>)taskData.getFiltering(), false);
			} else {
				result = clientAgeGenderDataToMariaDB.intoMariaClientAgeGenderDataV3(taskData.getId(), (List<BaseCVData>)taskData.getFiltering(), true);
			}
		} catch (Exception e) {
