...
import com.mobon.billing.core.service.ClickViewOpenrtbDataToMariaDB;
...
@Component
public class TaskClickViewOpenrtbData {

	private static final Logger		logger		= LoggerFactory.getLogger(TaskClickViewOpenrtbData.class);
	
	@Autowired
	private ClickViewOpenrtbDataToMariaDB	clickviewOpenrtbDataToMariaDB;

	@Autowired
	private RetryConfig				retryConfig;

	@Autowired
	@Qualifier("ClickViewOpenrtbDataWorkQueue")
	...
	public static void setThreadCnt(int threadCnt) {
		TaskClickViewOpenrtbData.threadCnt = threadCnt;
	}
	public static synchronized void increaseThreadCnt() {
		TaskClickViewOpenrtbData.threadCnt++;
	}
	...
	public static int getThreadCnt() {	
		return TaskClickViewOpenrtbData.threadCnt;
	}
	
	
	
	public void mongoToFileClickViewOpenrtb() {
		Map<String, BaseCVData> summeryBaseCVData = (Map<String, BaseCVData>) sumObjectManager.removeOpenrtbObjectMap();
		...
		if(summeryBaseCVData!=null && summeryBaseCVData.entrySet()!=null) {
			...
			if( listWriteData.size()>0 ) {
				...
				try {
					ConsumerFileUtils.writeLine( logPath +"retry/", writeFileName, G.click_view_openrtb, listWriteData);
				} catch (IOException e) {
				}
			}
		}
	...
	public void mongoToMariaClickViewOpenrtbV3() {
		...
		if(summeryBaseCVData!=null && summeryBaseCVData.entrySet()!=null) {
			...
			for (String group : group_key) {
				...
				List<BaseCVData> filtering = (List<BaseCVData>) listBaseCVData.stream()
						.filter(row -> _id.equals( row.getGroupingSeq() ) )
				...
				if( filtering.size()>0 ) {
					...
					if ( workingKey.contains(_id) ) {
						...
						for( BaseCVData row : filtering ) {
							sumObjectManager.appendOpenrtbData(row);
						}
					} else {
						...
						workQueue.execute(new TaskData(G.click_view_openrtb, _id, intoFiltering));
					}
				}
			}
		}
	...
	public void mongoToMariaV3(TaskData taskData) {
		...
		try {
			if ( taskData.increaseRetryCnt() < retryConfig.maxRetryCount ) {
				result = clickviewOpenrtbDataToMariaDB.intoMariaClickViewDataV3(taskData.getId(), (List<BaseCVData>)taskData.getFiltering(), false);
			} else {
				result = clickviewOpenrtbDataToMariaDB.intoMariaClickViewDataV3(taskData.getId(), (List<BaseCVData>)taskData.getFiltering(), true);
			}
		} catch (Exception e) {
