...
@Repository
public class ClickViewPcodeDataDao {

	private static final Logger	logger	= LoggerFactory.getLogger(ClickViewPcodeDataDao.class);

	public static final String	NAMESPACE	= "clickViewPcodeDataMapper";
			...
			public Object doInTransaction(TransactionStatus status) {
				try {
					...
					for (Entry<String, ArrayList<BaseCVData>> item : flushMap.entrySet()) {
						...
						logger.debug("clickviewPointData dataKey - {}", dataKey);
						logger.debug("clickviewPointData dataSize - {}", dataSize);
					...
					if(totalBillingSize>0) {
						...
						logger.info("TR Time (TBRT) Billing clickviewPointData : " + resutTime + "(ms) totalsize - "+ totalBillingSize);
					}
