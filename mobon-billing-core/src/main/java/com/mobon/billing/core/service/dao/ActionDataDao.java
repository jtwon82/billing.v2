...
public class ActionDataDao {
	...
	private static final Logger	logger	= LoggerFactory.getLogger(ActionDataDao.class);
			...
			public Object doInTransaction(TransactionStatus status) {
				...
				try {
					...
					for(Entry<String, ArrayList<ActionLogData>> item : flushMap.entrySet() ){
						...
						logger.debug("ACTION_LOG dataKey - {}", dataKey);
						logger.debug("ACTION_LOG dataSize - {}", itemSize);
			        ...
			        if(totalsize>0) {
						...
						logger.info("TR Time (TBRT) Dream ActionData : " + resutTime + "(ms) totalsize - "+ totalsize);
			        }
