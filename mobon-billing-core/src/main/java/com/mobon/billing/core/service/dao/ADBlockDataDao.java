...
@Repository
public class ADBlockDataDao {

	private static final Logger	logger	= LoggerFactory.getLogger(ADBlockDataDao.class);
			...
			public Object doInTransaction(TransactionStatus status) {
				try {
					...
					for (Entry<String, ArrayList<BaseCVData>> item : flushMap.entrySet()) {
						...
						logger.debug("ADBLOCK dataKey - {}", dataKey);
						logger.debug("ADBLOCK dataSize - {}", dataSize);
...
//							}
					...
					if(totalBillingSize>0) {
						...
						logger.info("TR Time (TBRT) Billing ADBLOCK : " + resutTime + "(ms) totalsize - "+ totalBillingSize);
					}
