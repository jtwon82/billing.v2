...
@Repository
public class ClickViewPoint2DataDao {

	private static final Logger	logger	= LoggerFactory.getLogger(ClickViewPoint2DataDao.class);
			...
			public Object doInTransaction(TransactionStatus status) {
				try {
					...
					for (Entry<String, ArrayList<BaseCVData>> item : flushMap.entrySet()) {
						...
						logger.debug("CLICKVIEWPoint2 dataKey - {}", dataKey);
						logger.debug("CLICKVIEWPoint2 dataSize - {}", dataSize);
					...
					if( totalDreamSize>0 ) {
						...
						logger.info("TR Time (TBRT) Dream ClickViewPoint2 : " + resutTime + "(ms) totalsize - "+ totalDreamSize);
					}
