package com.mobon.billing.branchAlgo.schedule;
...
import com.mobon.billing.branchAlgo.handler.ContextClosedHandler;
					...
					try {
						...
						while( (line = reader.readLine()) != null){
							...
							if( contextClosedHandler.closed ) {
								...
								if( G.click_view_pcode.equals( topic ) ) {
									try {
								...
								} else if (G.AlgoViewData.equals(topic)) {
									try {
																				
										sumObjectManager.appendViewPcodeData(item);

									}catch(Exception e) {
										writeFileName = String.format("%s_%s", "insertIntoErrorScriptNo", DateUtils.getDate("yyyyMMdd_HHmm"), millis);
										logger.error("err data - {}", jJSONObject.toString().substring(0,  200), e);
										ConsumerFileUtils.writeLine( logPath +"retryfail/", writeFileName, G.AlgoViewData, jJSONObject);
									}
								} else if(G.ABPcodeRecomData.equals(topic)) {
									try {

										sumObjectManager.appendABPcodeRecom(item);

									}catch(Exception e) {
										writeFileName = String.format("%s_%s", "insertIntoErrorScriptNo", DateUtils.getDate("yyyyMMdd_HHmm"), millis);
										logger.error("err data - {}", jJSONObject.toString().substring(0,  200), e);
										ConsumerFileUtils.writeLine( logPath +"retryfail/", writeFileName, G.ABPcodeRecomData, jJSONObject);
									}
								}
							}
						}
