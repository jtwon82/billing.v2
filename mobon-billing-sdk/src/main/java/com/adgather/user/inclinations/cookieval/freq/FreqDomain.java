 ...
 *
 */
public class FreqDomain implements CookieVal{
	/** values ****************************************/
	...
	public FreqDomain(String cookieValue) throws Exception {
		setCookieValue(cookieValue);
	}
	
	public FreqDomain(Document doc) throws Exception {
		setMongoValue(doc);
	}
	...
	public Object getCookieValue() {
		return String.format("%s%s%d", StringUtils.defaultString(domain), DELIMETER, freqCnt);
	}
	...
	@Override
	public FreqDomain clone() throws CloneNotSupportedException {
		FreqDomain newObj = new FreqDomain();
	...
	public void modValue(Object element, boolean bAppendValue) {
		if(!(element instanceof FreqDomain))		return;
		
		FreqDomain obj = (FreqDomain)element;
		if (bAppendValue) {
