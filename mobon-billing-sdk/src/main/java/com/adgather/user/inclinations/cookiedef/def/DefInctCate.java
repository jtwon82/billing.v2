 ...
 *
 */
public class DefInctCate extends CookieDef {
	public DefInctCate(String cookieKey) {
		super(cookieKey);
	}
	...
	public int getExpire() {
		return  PropertyHandler.getInt("COOKIE_INCT_CATE_EXPIRE");		// 6개월 고정 (ic_hu, ic_um, _ic_ki, tag, cat, gender, age 동일 적용)
	}
	
	@Override
	public void infuseLimit(MemoryObj<?> memoryObj) {
		memoryObj.setMongoMaxCnt(PropertyHandler.getInt("MONGO_INCT_CATE_COUNT"));
		memoryObj.setMongoMaxLen(PropertyHandler.getInt("MONGO_INCT_CATE_BYTES"));
		memoryObj.setCookieMaxCnt(PropertyHandler.getInt("COOKIE_INCT_CATE_COUNT"));
		memoryObj.setCookieMaxLen(PropertyHandler.getInt("COOKIE_INCT_CATE_BYTES"));
	}
	...
	public boolean isUseMongo() {
		return PropertyHandler.isTrue("MONGO_INCT_CATE_ENABLED");
	}
