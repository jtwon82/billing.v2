 ...
 *
 */
public class RefactStringLSyncList extends StringLSyncList implements RefactCookieList {
	private static final long serialVersionUID = 1L;
	private static final Logger logger = Logger.getLogger(RefactStringLSyncList.class);
	...
	public RefactStringLSyncList(String oldDelimeter, String newDelimeter ) {
		super(newDelimeter);
		...
		for (String element : arrayValue) {
			...
			try {
				if(cookieDef.getCodeConverter() != null) {
					element = cookieDef.getCodeConverter().decode(element);
				}
...
*/
	...
	public Map<String, String> getRefactCookieValue(int maxLen,	RefactCookieDef refactCooieDef) throws Exception {
		Map<String,StringLSyncList> syncLists = _getRefactSyncLists(refactCooieDef.getRefactCookieKey());
		
		Map<String, String> cookieMap = new HashMap<String, String>();
		for (Map.Entry<String, StringLSyncList> entry : syncLists.entrySet()) {
			StringLSyncList syncList = entry.getValue();
			if(syncList.size() > 0) {
	...
	public Map<String, Object> getRefactMongoValue(RefactCookieDef refactCooieDef) throws Exception {
		Map<String, StringLSyncList> syncLists = _getRefactSyncLists(refactCooieDef.getRefactMongoKey());
	...
	@Override
	public RefactStringLSyncList clone() {
		RefactStringLSyncList newList = new RefactStringLSyncList(OLD_DELIMETER, DELIMETER);
	...
	/** 쿠키/몽고 변경 객체 생성 **/
	private Map<String, StringLSyncList> _getRefactSyncLists(String refactKey) {
		if(refactKey == null)	return null; 
		
		Map<String, StringLSyncList> map = new HashMap<String, StringLSyncList>();
		
		StringLSyncList list = new StringLSyncList(DELIMETER);		// iUm
