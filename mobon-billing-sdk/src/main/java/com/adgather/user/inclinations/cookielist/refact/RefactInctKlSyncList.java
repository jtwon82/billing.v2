 ...
 *
 */
public class RefactInctKlSyncList extends ObjectLSyncList<OldInctKl> implements  RefactCookieList {

	private static final long serialVersionUID = 1L;
	private static final Logger logger = Logger.getLogger(RefactInctKlSyncList.class);
	...
	public RefactInctKlSyncList(String oldDelimeter, String newDelimeter ) {
		super(newDelimeter);
	...
	public boolean applyAdd(Object element, boolean bAppendValue, int maxCnt) {
		if(!(element instanceof InctKl))	return false;
		
		return super.applyAdd(new OldInctKl((InctKl)element), bAppendValue, maxCnt);
	}
	...
	public boolean applyDel(Object element) {
		...
		return super.applyDel(new OldInctKl((InctKl)element));
	}
	...
	public boolean applyMod(Object element, boolean bAppendValue) {
		...
		return super.applyMod(new OldInctKl((InctKl)element), bAppendValue);
	}
...
*/
	...
	public Map<String, String> getRefactCookieValue(int maxLen, RefactCookieDef refactCookieDef) throws Exception {
		Map<String, ObjectSyncList<InctKl>> syncLists = _getRefactSyncLists(refactCookieDef.getRefactCookieKey());
		
		Map<String, String> cookieMap = new HashMap<String, String>();
		for (Map.Entry<String, ObjectSyncList<InctKl>> entry : syncLists.entrySet()) {
			ObjectSyncList<InctKl> syncList = entry.getValue();
			if(syncList.size() > 0) {
	...
	public Map<String, Object> getRefactMongoValue(RefactCookieDef refactCookieDef) throws Exception {
		Map<String, ObjectSyncList<InctKl>> syncLists = _getRefactSyncLists(refactCookieDef.getRefactMongoKey());
	...
	@Override
	public RefactInctKlSyncList clone() {
		RefactInctKlSyncList newList = new RefactInctKlSyncList(OLD_DELIMETER, DELIMETER);
		...
		try {
			for (int idx = 0; idx < size(); idx++) {
				newList.add((OldInctKl)get(idx).clone());
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	/** 쿠키/몽고 변경 객체 생성 **/
	private Map<String, ObjectSyncList<InctKl>> _getRefactSyncLists(String cookieName) {
		if(cookieName == null)	return null;
		
		Map<String, ObjectSyncList<InctKl>> map = new HashMap<String, ObjectSyncList<InctKl>>();
		
		ObjectSyncList<InctKl> list = new ObjectSyncList<InctKl>(DELIMETER);		// iKl
		list.setCurTime(this.curTime);
		list.setSyncTime(this.syncTime);
		if(size() > 0) {
			for(int idx = 0; idx < size(); idx++) {
				OldInctKl oldInctKl = get(idx);
				list.add(new InctKl(oldInctKl));
			}
		}
