...
DROP PROCEDURE IF EXISTS `sp_scn_click2_point`$$

CREATE PROCEDURE `sp_scn_click2_point`(
...
BEGIN
	...
	IF $POINT>0 THEN
		...
		UPDATE iadsite_TEST SET usedmoney=usedmoney+$POINT WHERE site_code=$site_code;
	END IF;
