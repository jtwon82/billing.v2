...
DROP PROCEDURE IF EXISTS `sp_sky_view_point`$$

CREATE PROCEDURE `sp_sky_view_point`(
...
BEGIN
	...
	IF $POINT>0 THEN
		...
		UPDATE iadsite_TEST SET usedmoney=usedmoney+$POINT WHERE site_code=$site_code AND svc_type='sky';
		UPDATE iadsite_TEST SET usedmoney=usedmoney+$POINT WHERE site_code_s=$site_code AND svc_type='sky' AND gubun=$ad_gubun;
	END IF;
