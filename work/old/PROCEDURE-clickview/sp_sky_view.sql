...
DROP PROCEDURE IF EXISTS `sp_sky_view`$$

CREATE PROCEDURE `sp_sky_view`(
...
BEGIN
	...
	-- IF $POINT>0 THEN
	-- 	UPDATE admember SET POINT=POINT - $POINT, lastupdate=NOW() WHERE userid=$advertiser_id;
	-- 	UPDATE iadsite SET usedmoney=usedmoney+$POINT WHERE site_code=$site_code AND svc_type='sky';
	-- 	UPDATE iadsite SET usedmoney=usedmoney+$POINT WHERE site_code_s=$site_code AND svc_type='sky' AND gubun=$ad_gubun;
	-- END IF;
