...
import com.mobon.billing.framertb.consumer.ConsumerProcess;
import com.mobon.billing.framertb.handler.ContextClosedHandler;
import com.mobon.billing.framertb.model.PollingData;
import com.mobon.billing.framertb.service.SumObjectManager;
import com.mobon.billing.framertb.service.WorkQueue;
...
//										FrameRtbData item = FrameRtbData.fromHashMap(jJSONObject);
//										
...
//											sumObjectManager.appendFrameCombiDatStats(item);
//										} else {
//											sumObjectManager.appendFrameCycleLog(item);
//											sumObjectManager.appendFrameDayStats(item);
//											sumObjectManager.appendFrameTrnLog(item);
//											sumObjectManager.appendFrameMediaAdverStats(item);
//											
//											if (this.processFrameAdverDayStats(item)) {
//												sumObjectManager.appendFrameAdverDayStats(item);												
//											}
//											
//											if (this.processFrameAdverPrdtCtgrDayStats(item)) {
//												sumObjectManager.appendFrameAdverPrdtCtgrDayStats(item);;												
//											}
//											
//										}
//										if(item.getAdverProdData() != null) {
//											sumObjectManager.appendFrameSizeStats(item);
//											if ("C".equals(item.getType())) {
//												sumObjectManager.appendFrameActionLog(item);
//											}
//										}
										consumerProcess.processMain(topic,  jJSONObject.toString());
										
									}catch(Exception e) {
										logger.error("err data - {}", jJSONObject.toString().substring(0,  200), e);
										ConsumerFileUtils.writeLine( logPath +"retryfail/", writeFileName, G.framertb_info, jJSONObject);
									}
								}
							}
						}
