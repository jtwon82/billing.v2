...
import com.mobon.billing.sample.handler.ContextClosedHandler;
...
import com.mobon.billing.sample.service.WorkQueue;
import com.mobon.billing.sample.util.ConsumerFileUtils;
import com.mobon.billing.sample.util.FileUtils;
					...
					try {
						...
						while( (line = reader.readLine()) != null){
							...
							if( contextClosedHandler.closed ) {
								...
								if( "SampleVo".equals( topic ) ) {
									try {
										SampleVo item = SampleVo.fromHashMap(jJSONObject);
										
										sumObjectManager.appendSampleVo(item);
										
									}catch(Exception e) {
										logger.error("err data - {}", jJSONObject.toString().substring(0,  200), e);
										ConsumerFileUtils.writeLine( logPath +"retryfail/", writeFileName, "SampleVo", jJSONObject);
									}
								}
								else if( "UseridVo".equals( topic ) ) {
									try {
//										UseridVo item = UseridVo.fromHashMap(jJSONObject);
										PollingData obj = new ObjectMapper().readValue(msg, PollingData.class);
										
										sumObjectManager.appendUseridVo(obj.toUseridVo());
										
									}catch(Exception e) {
										logger.error("err data - {}", jJSONObject.toString().substring(0,  200), e);
										ConsumerFileUtils.writeLine( logPath +"retryfail/", writeFileName, "UseridVo", jJSONObject);
									}
								}
							}
						}
