...
import com.mobon.billing.report.handler.ContextClosedHandler;
import com.mobon.billing.report.service.SumObjectManager;
import com.mobon.billing.report.service.WorkQueue;
							...
							if( contextClosedHandler.closed ) {
								...
								try {
									if( G.NativeNonAdReport.equals( topic ) ) {
										try {
											NativeNonAdReportData item = NativeNonAdReportData.fromHashMap(jJSONObject);
											
											item.setPlatform(G.convertPLATFORM_CODE_BACK(item.getPlatform()));
											item.setAdGubun(G.convertTP_CODE_BACK(item.getAdGubun()));
											
											sumObjectManager.appendNativeNonAdReportData(item);
