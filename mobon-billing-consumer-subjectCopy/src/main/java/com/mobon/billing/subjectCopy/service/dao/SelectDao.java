...
public class SelectDao {
	...
	public static final String	NAMESPACE	= "selectSubjectCopyMapper";
	
	public BaseCVData selectMediaInfo(ClickViewData data) {
		BaseCVData result = null;
		try {
			result = sqlSessionTemplateBilling.selectOne(String.format("%s.%s", NAMESPACE, "selectMediaInfo"), data);
	...
	public BaseCVData selectMediaScriptChrgInfo(ClickViewData data) {
		...
		try {
			result = sqlSessionTemplateBilling.selectOne(String.format("%s.%s", NAMESPACE, "selectMediaScriptChrgInfo"), data);
		}catch(Exception e) {
	...
	public BaseCVData selectOpenRtbHIRNK_NO(BaseCVData data) {
		BaseCVData result = sqlSessionTemplateBilling.selectOne(String.format("%s.%s", NAMESPACE, "selectOpenRtbHIRNK_NO"), data);
		return result;
	}
	...
	public ArrayList selectAdgubunKey2() {
		return (ArrayList) sqlSessionTemplateBilling.selectList(String.format("%s.%s", NAMESPACE, "selectAdgubunKey2"));
	}

	public ArrayList selectAdgubunKey2_EX() {
		return (ArrayList) sqlSessionTemplateBilling.selectList(String.format("%s.%s", NAMESPACE, "selectAdgubunKey2_EX"));
	}
	...
	public Map<String, String> selectMobonComCode(String CODE_TP_ID) {
		...
		try {
			...
			List<Map> list = this.sqlSessionTemplateBilling.selectList(String.format("%s.%s", NAMESPACE, "selectMobonComCode"), param);
	...
	public String selectMobonComCodeAdvrtsPrdtCode(String product) {
		...
		try {
			...
			List<Map> list = this.sqlSessionTemplateBilling.selectList(String.format("%s.%s", NAMESPACE, "selectMobonComCodeAdvrtsPrdtCode"));
	...
	//kpi INFO 
	public BaseCVData selectKpiInfo(BaseCVData vo) {
		BaseCVData result = sqlSessionTemplateBilling.selectOne(String.format("%s.%s", NAMESPACE, "selectNewKpiStats"),vo);
		return result;
	}
	...
	//unique click 데이터 를 가지고오기위한 메소드 
	public List<Map<String, Object>> getUniqueClickData(Map<String, Object> param) {
		
		List<Map<String, Object>> result = sqlSessionTemplateBilling.selectList(String.format("%s.%s",NAMESPACE, "getUniqueClickData"), param);				
